...
interface ParkingDetails {
    ...
    tags: {
        ...
        amenity?: string;
        fee?: string;
        parking?: string; // "underground" or other values
        capacity?: string;
        "addr:city"?: string;
        "addr:street"?: string;
    };
...
interface ParkingProps {
    isLoggedIn: boolean; // Passed from App to check login status
    token: string; // Token for authenticated API requests
}
...
const Parking: React.FC<ParkingProps> = ({ isLoggedIn, token }) => {
    ...
    const handleConfirmParking = async () => {
        ...
        try {
            const duration = (new Date(endTime).getTime() - new Date(startTime).getTime()) / 60000; // Duration in minutes
    ...
    return (
        <div style={styles.container}>
            ...
            <div style={styles.details}>
                ...
                <p><strong>Name:</strong> {parkingData?.tags?.name || 'Unknown'}</p>
                <p><strong>City:</strong> {parkingData?.tags?.['addr:city'] || 'Not specified'}</p>
                <p><strong>Street:</strong> {parkingData?.tags?.['addr:street'] || 'Not specified'}</p>
                <p><strong>Capacity:</strong> {parkingData?.tags?.capacity || 'Unknown'}</p>
                <p><strong>Fee:</strong> {parkingData?.tags?.fee || 'Unknown'}</p>
                <p><strong>Type:</strong> {parkingData?.tags?.parking === 'underground' ? 'Garažna hiša' : 'Parkirišče'}</p>
            </div>
            ...
            {showPrompt && (
                <div style={styles.promptOverlay}>
                    <div style={styles.prompt}>
                        ...
                        <button onClick={() => navigate('/login')} style={styles.promptButton}>
                            Go to Login
                        </button>
